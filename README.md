...
| 行为 | 说明 |
|---|---|
| 🏃 满屏跑动 | **整屏任意位置都能去**：在底部漫步、奔跑，也会一跃而起蹦到屏幕上半空悬停玩耍，累了再落回地面 |
| 🎈 空中悬停 | 悬在半空时会「踩空气」（四肢轻轻划动）+ 上下浮动 + 吐舌头，落回地面时抖抖毛 |
...
| 📌 托盘菜单 | 右键小狗或托盘图标：显示/隐藏、暂停、指定动作、大小、**满屏跑动/只在底部**、置顶、开机自启、退出 |
...
| `dist/PuppyPetTests.exe` | 自测程序（47 项检查 + 姿态图/肖像/图标生成） |

## 自测

```cmd
dist\PuppyPetTests.exe                                :: 跑 47 项断言
...
自测覆盖：初始状态、悬停变开心、摸头/挠痒来回切换、**鼠标移开 10 秒后偷偷溜走**、
**满屏跑动（会跑到屏幕上半部分、悬停时有浮动、玩够了自己回地面）**、只在底部模式恒在地面线、
长时间无人看管时自主活动且不越界、拖动→下落→落地→抖毛、点击起跳、卡顿保护、暂停、
同种子轨迹可复现、命中测试，以及 12 种姿态的绘制（覆盖率、不溢出画布、左右转身镜像一致）、
正面肖像的绘制与「黑色五官占比」校验。

## 交互说明

- **摸头 / 挠痒**：把鼠标移到小狗身上即可，不需要点击。
- **偷偷跑掉**：鼠标离开后 10 秒。想让它留下就再摸一下（计时会重置）。半空中被发现要走，它会直接飞走。
- **满屏 / 只在地面**：默认满屏跑动（在地面待够 14 秒必定蹦一次，不会一直赖在底部）；
  想要安静点就在右键菜单里关掉「满屏跑动」，它就只在屏幕底部活动。
- **拎起来**：左键按住拖动，松手掉落。
- **菜单**：在狗身上点右键，或点托盘图标。
...
- **二维活动**：小狗有完整的 (x, y) 与二维机动——地面走跑，空中用抛物线起跳 + 平滑转向，
  悬停时叠加正弦浮动（`VisualY`），所以"满屏跑"看起来是蹦上去的，而不是凭空飘。
